...
{
    private final ArrayList<Horde> hordes = new ArrayList<>();
    private final ArrayList<HordeMember> hordeMembers = new ArrayList<>();
    
    public Boolean HasHorde(Player player)
    {
        if (hordeMembers.stream().anyMatch((member) -> (member.getPlayerUUID().equals(player.getUniqueId().toString())))) {
            return true; 
        }
        return false;
    }
    ...
    {
        int count = 0;
        count = hordeMembers.stream().filter((member) -> (member.getHordeName().equals(hordeName) && member.getTypeMember() == grade)).map((_item) -> 1).reduce(count, Integer::sum);
        return count;
    }
    ...
    {
        int count = 0;
        count = hordeMembers.stream().filter((member) -> (member.getHordeName().equals(h.getHordeName()))).map((_item) -> 1).reduce(count, Integer::sum);
        return count;
    }
